...
- `3.1 SELECT ...`：看不带 `FROM` 时能查询什么。
- `3.2 SELECT ... FROM`：看最基础的表查询结构，以及 `SELECT *` 和指定列的差异。
- `3.3 列的别名`：看如何给列或表达式起更好理解的名字。
- `3.4 去除重复行`：看 `DISTINCT` 的位置和作用范围。
- `3.5 空值参与运算`：看 `NULL` 参与计算时为什么结果常常还是 `NULL`。
- `3.6 着重号`：看对象名和保留字冲突时怎么写。
- `3.7 查询常数`：看如何在结果集中增加固定值列。
- `常见混淆点`：看最容易在练习时写错或误解的地方。

## 建议阅读顺序

- 第一次学习时，建议按 `3.1 -> 3.2 -> 3.3 -> 3.4 -> 3.5 -> 3.6 -> 3.7` 的顺序阅读，先掌握基本查询骨架，再处理细节问题。
- 如果你只是想写最简单的查询，先看 `3.2 SELECT ... FROM` 就够用。
- 如果你是查询结果看起来不对，优先回查 `3.3 列的别名`、`3.4 去除重复行` 和 `3.5 空值参与运算`。
- 如果你遇到语法报错而且对象名看起来没问题，优先回查 `3.6 着重号`。
...
这一小节先回答一个最容易忽略的问题：`SELECT` 不一定非要配合数据表，它也可以单独用来验证常量或表达式。
...
如果你只是想快速看一个表达式的结果，而不是查表数据，可以直接写不带 `FROM` 的 `SELECT`。

## 3.2 SELECT ... FROM

这一小节开始进入最常见的查询骨架：`SELECT` 负责决定查什么，`FROM` 负责决定从哪张表查。
...
写查询时先问自己两件事：要查哪些列、这些列来自哪张表。大多数基础查询都能归结为这个结构。
...
如果查询结果列名太长、不直观，或者某一列其实是计算结果，优先考虑使用别名改善可读性。
...
看到重复行时，不要急着认为数据库有问题。先确认自己是不是需要 `DISTINCT`，以及它应该作用在哪几列的组合上。

## 3.5 空值参与运算

这一小节要处理的是查询里一个很常见的坑：只要表达式里出现 `NULL`，运算结果往往就不是你直觉以为的数值。
...
如果计算结果整列都是空，优先检查参与运算的字段里是不是存在 `NULL`。
...
如果对象名看起来没错、但语法还是报错，检查它是不是和关键字重名，以及是否需要用着重号包裹。
...
如果你需要在结果里标记数据来源、课程示例来源或分组来源，查询常数会比后处理更直接。

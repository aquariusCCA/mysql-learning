...
这篇内容偏操作型，适合在“安装失败需要重装”时查阅；如果是第一次阅读，建议按 `操作前检查 -> 步骤1 -> 步骤2 -> 步骤3` 的顺序走，只有在普通卸载后仍然安装失败时，再继续看 `步骤4` 和 `步骤5`。
...
- `操作前检查` 备份数据与区分程序目录、数据目录
- `步骤1` 停止 MySQL 服务
- `步骤2` 执行程序卸载
- `步骤3` 清理残余文件
- `步骤4` 清理注册表
- `步骤5` 删除环境变量配置
- `常见风险点` 卸载和重装时最容易出错的地方
...
- 第一次操作时，先看 `操作前检查`，确认是否要保留旧数据，再按 `步骤1` 到 `步骤3` 执行标准卸载流程。
- 只有在重新安装仍然失败时，再继续看 `步骤4` 和 `步骤5`，处理注册表和环境变量。
- 如果你卸载完成后要重新安装 MySQL，可以继续看 [2 MySQL 的下载、安装、配置](./2%20MySQL%20的下载、安装、配置.md)。
...
在卸载之前，先停止 `MySQL8.0` 的服务。按键盘上的 **Ctrl + Alt + Delete** 组合键，打开 **任务管理器** 对话框，可以在 **服务** 列表找到 **MySQL8.0** 的服务；如果现在是 **正在运行** 状态，可以右键单击服务，选择 **停止** 选项停止 MySQL8.0 的服务，如图所示。
...
卸载 `MySQL8.0` 的程序可以和其他桌面应用程序一样直接在 **控制面板** 选择 **卸载程序**，并在程序列表中找到 `MySQL8.0` 服务器程序，直接双击卸载即可，如图所示。这种方式删除时，数据目录下的数据不会跟着删除。
...
- ③ 单击 **Next**（下一步）按钮，确认卸载。
  ...
  ![确认卸载步骤](./images/image-20211014153747283.png)
...
> ⚠️ 注意：
> - 请在卸载前做好数据备份。
> - 在操作完以后，需要重启计算机，然后再进行安装。
> - 如果仍然安装失败，再继续执行下面的 `步骤4`。

## 步骤4：清理注册表（选做）

如果前几步做完之后，再次安装还是失败，那么可以清理注册表。
...
> ✏️ 注册表中的 `ControlSet001`、`ControlSet002` 不一定就是 `001` 和 `002`，也可能是 `ControlSet005`、`ControlSet006` 之类。

## 步骤5：删除环境变量配置

找到 `Path` 环境变量，将其中关于 MySQL 的环境变量删除，**切记不要全部删除。**

例如：删除 `D:\develop_tools\mysql\MySQLServer8.0.26\bin;` 这个片段。

![删除 Path 中的 MySQL 路径](./images/1575694476072.png)

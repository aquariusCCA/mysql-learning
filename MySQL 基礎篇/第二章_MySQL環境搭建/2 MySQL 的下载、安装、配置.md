...
> 建议回查情境：需要重新下载安装包、忘记 MySQL8.0 配置步骤、命令行无法直接使用 `mysql`、安装失败需要确认依赖或残留服务时

## 本节导读

这一节主要说明 MySQL 在 Windows 下的下载、安装、初始化配置、环境变量设置，以及安装失败时常见的排查方向。

内容比较长，第一次阅读建议按“版本选择 → 安装 → 配置 → 环境变量 → 常见问题”顺序看；如果只是回头复习，可以先根据小节标题定位到需要的步骤。

安装完成后，可以继续阅读 [3 MySQL 的登录](./3%20MySQL%20的登录.md)；如果过程中遇到环境变量、编码或报错问题，也可以回查 [7 常见问题的解决（课外内容）](./7%20常见问题的解决(课外内容).md)。

## 你会在这篇学到什么

- MySQL 常见版本之间的差异，以及课程为什么使用 `8.0.26`。
- Windows 下推荐下载哪一种 MySQL 安装包。
- 如何通过 MSI 安装程序安装 MySQL8.0。
- 初始化配置时端口号、授权方式、`root` 密码和 Windows 服务名分别代表什么。
- 如何把 MySQL 的 `bin` 目录加入 `Path` 环境变量。
- 安装失败时应优先检查哪些依赖、残留目录和残留服务。
...
- 原教材当时说明的最新版本为 `8.0.27`，发布时间为 `2021年10月`。此前，8.0.0 在 2016.9.12 日就发布了。
- 本课程中使用`8.0.26版本`。
...
步骤5：单击【编辑】按钮，在【编辑环境变量】对话框中，将 MySQL 应用程序的 bin 目录（`C:\Program Files\MySQL\MySQL Server 8.0\bin`）添加到变量值中，用分号将其与其他路径分隔开。
...
- 安装时为什么建议选择 `Custom`？
- 配置 MySQL8.0 时，`Config Type`、端口号、授权方式和服务名分别在哪里设置？
- 为什么命令行不能直接输入 `mysql`？
- 安装失败时，应该先检查依赖、残留目录还是残留服务？

## 一句话抓核心

Windows 下搭建 MySQL 环境的核心顺序是：先选对安装包，再完成服务器安装与初始化配置，最后把 `bin` 目录加入 `Path`，并在失败时按依赖、残留目录、残留服务逐步排查。

## 小结

这一节你需要记住：

- Windows 初学阶段优先选择 MSI 安装程序，课程示例使用 `mysql-installer-community-8.0.26.0.msi`。
- MySQL8.0 安装时可以通过 `Custom` 自定义安装组件、服务目录和数据目录。
- 配置阶段重点确认端口号 `3306`、授权方式、`root` 密码和 Windows 服务名 `MySQL80`。
- 配置 `Path` 后，才能在命令行中更方便地直接使用 `mysql` 命令。
- 安装失败时不要盲目重装，先检查 `.NET Framework`、Visual C++ 运行库、残留安装目录、数据目录和残留服务。
